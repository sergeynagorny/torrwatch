...
const Select = ({ value, ...props }: SelectPrimitive.SelectProps) => {
  // https://github.com/radix-ui/primitives/issues/1569#issuecomment-1434801848
  const key = useMemo(() => value || Date.now(), [value]);

  return <SelectPrimitive.Root key={key} value={value} {...props} />;
};
...
>(({ className, children, position = 'popper', ...props }, forwardedRef) => {
  ...
  const handleRef = (ref: never) => {
    // Attach the touchend event listener
    if (ref) {
      // @ts-ignore
